...
def combos(imagelist,cursor):
    ...
    temptuple = tuple([x[0] for x in perms(imagelist)]+[x[1] for x in perms(imagelist)])

    templist = []

    for i in xrange(0,len(temptuple)/2,1000):
        temptemptuple = temptuple[i:i+100]+temptuple[i+len(temptuple)/2-1:i+100+len(temptuple)/2-1]
        templist += cursor.execute('SELECT image_one,image_two,"11,64" FROM ssim WHERE image_one IN (' + ','.join('?'*(len(temptemptuple)/2)) + ')' + 'AND image_two IN (' + ','.join('?'*(len(temptemptuple)/2)) + ')', temptemptuple).fetchall()
...
def combos(imagelist,cursor):
    ...
    for i in xrange(len(imagelist)+1):
        for j in xrange(i+1,len(imagelist)):
            try:
                tempval = cursor.execute('SELECT "%s" FROM ssim WHERE image_one="%s" AND image_two="%s"' % (pair_dim,imagelist[i].path,imagelist[j].path)).fetchone()[0]
                if type(tempval) == float or tempval.encode('ascii') == 'na':
                    yield [imagelist[i],imagelist[j]]
                    continue
                else:
                    yield [imagelist[i],imagelist[j]]
...
def ssim(imagelist,macrosize,dimensions,thresholdlower,thresholdupper):
    ...
    ssim_list = pool.map_async(mssim_test, combos(imagelist,cursor), int(triangle(len(imagelist))/float(numprocesses))+numprocesses).get(None)
...
def imagein(path,macrosize,dimensions):
    ...
    count = 0
    imagelist = []
    dupefolder = re.compile('DUPES')
    dirlen = 0
    img = 0
    numprocesses = 2

    for root, dirs, files in os.walk(path):
        if len(re.findall(dupefolder,root)) == 0:
            dirlen += len(files)
...
def optimize(imagelist_in, multisize):
    ratio1,ratio2 = 0,0
    for img1index in xrange(len(imagelist_in)):
        for img2index in xrange(img1index+1,len(imagelist_in)):
            ...
            if (multisize == 1 and imagelist_in[img1index].h == imagelist_in[img2index].h and imagelist_in[img1index].w == imagelist_in[img2index].w):
                continue
            
            elif ((ratio1-ratio2)/ratio2)>aspectratio or ((ratio2-ratio1)/ratio1)>aspectratio:
                continue
